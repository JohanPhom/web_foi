    ...
    <section id="mainContent">
        <div class="background">
            <div id="box_article_fan_page">
                <img src="multimedia/{$club->image_title}" style="position:absolute; left:40%; top:25%;z-index:1;" alt="{$club->name} title" width="20%">
                <a href="index.php?page=fanPage&ID={$club->ID}"><div class="button" style="width:10%;text-align:center;">Back</div></a>
                <form id="form_test" action="index.php?page=fanPage&ID={$club->ID}&user={$user->ID}" method="POST">
                <h2>Test / Quizz</h2>
                {assign var=i value=1}
                {foreach $test as $tests}
                    <label for="question{$i}">Question {$i} : <b>{$tests}</b></label><br>
                    <input class="input_test" type="text" name="answer{$i}"><br>
