        ...
        <div class="background">
            <div id="log_box">
                ...
                <form action="index.php?page=log" method="POST">
                    ...
                    <a href="index.php?page=log{if $pagination > 0}&pagination={$log_pagination-1}{/if}"><img src="multimedia/white_left_arrow.png" alt="right_arrow" height="30px;"></a>
                    <a href="index.php?page=log&pagination={$log_pagination+1}"><img src="multimedia/white_right_arrow.png" alt="right_arrow" height="30px;"></a>
                    {for $i = 0 to ($nb_ligne / 20)-1}
                        <a href="index.php?page=log&pagination={$i}">{$i}</a>
                    {/for}
                </form>
                {if isset($log_file)}
                    {for $i=$log_pagination*20 to $log_pagination*20+20}
                        {if isset($log_file[$i])}
                            <p style="font-family:monospace">{$log_file[$i]}</p>
                        {/if}
                    {/for}
                {/if}
            </div>
